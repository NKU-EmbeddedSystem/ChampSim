#!/bin/bash
# Task 4.0: Build ChampSim binaries for 1-core and 4-core × 4 replacement policies
...
# Control plane
# ------------------------------------------------------------------
N_TOTAL=$((${#POLICIES[@]} * ${#CORE_CONFIGS[@]}))
cat > "$RUN_DIR/execution.log" << EOF
[$(date -u +%Y-%m-%dT%H:%M:%S)] STAGE BEGIN  stage=task4.0-build  tasks=$N_TOTAL  run=$RUN_TS
EOF

cat > "$RUN_DIR/main.log" << EOF
══════════════════════════════════════════════════════════
  Task 4.0: Build ChampSim Binaries
  Policies:    ${POLICIES[*]}
...
CHAMPSIM_H_BAK="$STAGE_DIR/inc/champsim.h.bak.4.0"
...
# ------------------------------------------------------------------
task_idx=0
for ncores in "${CORE_CONFIGS[@]}"; do
  echo "" | tee -a "$RUN_DIR/main.log"
  echo "=== Building ${ncores}-core binaries ===" | tee -a "$RUN_DIR/main.log"

  # Modify NUM_CPUS in champsim.h
  sed -i "s/^#define NUM_CPUS .*/#define NUM_CPUS ${ncores}/" "$CHAMPSIM_H"
  echo "  Set NUM_CPUS=$ncores in champsim.h" >> "$RUN_DIR/main.log"

  for pol in "${POLICIES[@]}"; do
    task_idx=$((task_idx + 1))
    bin_name="bimodal-no-no-no-no-${pol}-${ncores}core"
    log_file="$RUN_DIR/build_${pol}-${ncores}core.log"
    name="build_${pol}_${ncores}core"

    echo "[$(date -u +%Y-%m-%dT%H:%M:%S)] TASK DISPATCH  [$task_idx/$N_TOTAL] name=$name" >> "$RUN_DIR/execution.log"
    echo "── ${name} ─────────────────────────────────────────────" >> "$RUN_DIR/main.log"
    echo "  Binary: bin/${bin_name}" >> "$RUN_DIR/main.log"

    t0=$(date +%s%3N)

    if [ -f "$BIN_DIR/$bin_name" ]; then
      echo "  → Already exists, skipping build" | tee -a "$RUN_DIR/main.log"
      t1=$(date +%s%3N)
    ...
    if [ -f "$BIN_DIR/champsim" ]; then
      ...
      t1=$(date +%s%3N)
      echo "  → bin/${bin_name} built OK (${ncores}-core, policy=$pol)" | tee -a "$RUN_DIR/main.log"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%S)] STAGE DONE  stage=task4.0-build  ok=$ok_count  fail=$fail_count" >> "$RUN_DIR/execution.log"
