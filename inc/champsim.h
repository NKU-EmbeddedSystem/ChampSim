...
#define CXL_IO_FREQ                                                            \
  800 // CXL effective MT/s, 1/4 of DRAM for bandwidth modeling
...
#define DRAM_BANKS 32 // 64MB * 32 banks => 2GB per rank
#define LOG2_DRAM_BANKS 5
#define DRAM_ROWS 65536 // 2KB * 32K rows => 64MB per bank
#define LOG2_DRAM_ROWS 16
#define DRAM_COLUMNS                                                           \
  128 // 64B * 128 => 8KB per row (matches latest 1024 cols x 8B = 8KB)
...
#define CXL_SIZE                                                               \
  (CXL_CHANNELS * CXL_RANKS * CXL_BANKS * CXL_ROWS * CXL_ROW_SIZE / 1024)
...
constexpr uint64_t lg2(uint64_t n) { return n < 2 ? 0 : 1 + lg2(n / 2); }
#endif
