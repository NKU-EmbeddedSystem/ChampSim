...
public:
  uint64_t instr_id = 0, ip = 0, fetch_producer = 0, producer_id = 0,
           translated_cycle = 0, fetched_cycle = 0, execute_begin_cycle = 0,
           retired_cycle = 0, event_cycle = 0;

  uint8_t is_branch = 0, is_memory = 0, branch_taken = 0,
          branch_mispredicted = 0, branch_prediction_made = 0,
          source_added[NUM_INSTR_SOURCES] = {},
          destination_added[NUM_INSTR_DESTINATIONS_SPARC] = {}, is_producer = 0,
          is_consumer = 0, reg_RAW_producer = 0, reg_ready = 0, mem_ready = 0,
  ...
  uint64_t instruction_pa = 0, data_pa = 0, virtual_address = 0,
           physical_address = 0;
  ...
  ooo_model_instr() {
    std::fill(std::begin(lq_index), std::end(lq_index),
              std::numeric_limits<uint32_t>::max());
    std::fill(std::begin(sq_index), std::end(sq_index),
              std::numeric_limits<uint32_t>::max());
  };
  ...
  void print_instr() {
    ...
    std::cout << (uint32_t)is_branch << " " << (uint32_t)branch_taken
              << std::endl;
    for (uint32_t i = 0; i < NUM_INSTR_SOURCES; i++) {
    ...
    std::cout << std::endl;
    for (uint32_t i = 0; i < NUM_INSTR_DESTINATIONS; i++) {
      std::cout << std::hex << "0x" << (uint32_t)destination_memory[i]
                << std::dec << " ";
    }
    std::cout << std::endl;
    std::cout << std::endl;
