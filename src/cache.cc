    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
      ...
      if ((cache_type == IS_LLC) &&
        ...
        if (this->cache_type == IS_LLC) {
          int area = MSHR.entry[mshr_index].area;
          // 确保 area 是合法值 (0, 1, 2)
          if (area >= 0 && area < 3) {
            this->llc_miss_by_area[area]++;
            this->llc_access_by_area[area]++;
          }
        }
        // ===========================================================================
        ...
        if (do_fill) {
          ...
          if (cache_type == IS_L1D)
            l1d_prefetcher_cache_fill(
                MSHR.entry[mshr_index].full_v_addr,
          ...
          if (cache_type == IS_L2C)
            MSHR.entry[mshr_index].pf_metadata = l2c_prefetcher_cache_fill(
                (MSHR.entry[mshr_index].v_address) << LOG2_BLOCK_SIZE,
      ...
      if (way >= 0) { // writeback hit (or RFO hit for L1D)
        ...
        if (cache_type == IS_L1D) { // RFO miss
          ...
          if (do_fill) {
            ...
            if (cache_type == IS_L1D)
              l1d_prefetcher_cache_fill(
                  WQ.entry[index].full_v_addr, WQ.entry[index].full_addr, set,
                  way, 0, block[set][way].address << LOG2_BLOCK_SIZE,
                  WQ.entry[index].pf_metadata);
            else if (cache_type == IS_L2C)
              WQ.entry[index].pf_metadata = l2c_prefetcher_cache_fill(
                  (WQ.entry[index].v_address) << LOG2_BLOCK_SIZE,
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) &&
      ...
      if (way >= 0) { // read hit
        ...
        if (RQ.entry[index].type == LOAD) {
          ...
          if (cache_type == IS_L1D)
            l1d_prefetcher_operate(RQ.entry[index].full_v_addr,
                                   RQ.entry[index].full_addr,
                                   RQ.entry[index].ip, 1, RQ.entry[index].type);
          else if (cache_type == IS_L2C)
            l2c_prefetcher_operate(
                (RQ.entry[index].v_address) << LOG2_BLOCK_SIZE,
                (block[set][way].address) << LOG2_BLOCK_SIZE,
          ...
          if (cache_type == IS_LLC) {
            ...
            if (lower_level)
              ...
              if (cache_type == IS_STLB) {
                ...
                uint64_t pa =
                    vmem.va_to_pa(read_cpu, RQ.entry[index].full_addr);
          ...
          if (RQ.entry[index].type == LOAD) {
            ...
            if (cache_type == IS_L1D)
              l1d_prefetcher_operate(
                  RQ.entry[index].full_v_addr, RQ.entry[index].full_addr,
                  RQ.entry[index].ip, 0, RQ.entry[index].type);
            if (cache_type == IS_L2C)
              l2c_prefetcher_operate(
                  (RQ.entry[index].v_address) << LOG2_BLOCK_SIZE,
...
int CACHE::add_rq(PACKET *packet) {
  ...
  // ==================== 【新增代码：全局拦截介质分配】 ====================
  // 只在最顶层（L1 或 TLB）分配，避免下级 Cache 重复查表
  if (this->cache_type == IS_L1I || this->cache_type == IS_L1D ||
...
int CACHE::va_prefetch_line(uint64_t ip, uint64_t pf_addr, int pf_fill_level,
                            uint32_t prefetch_metadata) {
  if (pf_addr == 0) {
  ...
  if (VAPQ.occupancy < VAPQ.SIZE) {
    ...
    pf_packet.address =
        pf_addr >>
        LOG2_BLOCK_SIZE; // make address == v_address before translation just so
                         // we can use VAPQ's check_queue() function
...
void CACHE::va_translate_prefetches() {
  ...
  if (PQ.occupancy < PQ.SIZE) {
    for (uint32_t i = 0; i < VAPQ.SIZE; i++) {
      ...
      if ((VAPQ.entry[vapq_index].address != 0) &&
          (VAPQ.entry[vapq_index].address !=
           VAPQ.entry[vapq_index].v_address)) {
  ...
  for (uint32_t i = 0; i < VAPQ.SIZE; i++) {
    if ((VAPQ.entry[vapq_index].address == VAPQ.entry[vapq_index].v_address) &&
        (VAPQ.entry[vapq_index].event_cycle <= current_core_cycle[cpu])) {
      VAPQ.entry[vapq_index].full_addr =
          vmem.va_to_pa(cpu, VAPQ.entry[vapq_index].full_v_addr);
      VAPQ.entry[vapq_index].address =
          (VAPQ.entry[vapq_index].full_addr) >> LOG2_BLOCK_SIZE;
      break;
    }
    vapq_index++;
    if (vapq_index >= VAPQ.SIZE) {
  ...
  for (uint32_t i = 0; i < VAPQ.SIZE; i++) {
    if ((VAPQ.entry[vapq_index].address == VAPQ.entry[vapq_index].v_address) &&
        (VAPQ.entry[vapq_index].event_cycle == 0)) {
      VAPQ.entry[vapq_index].event_cycle =
          current_core_cycle[cpu] + VA_PREFETCH_TRANSLATION_LATENCY;
      break;
    }
    vapq_index++;
    if (vapq_index >= VAPQ.SIZE) {
