...
uint32_t O3_CPU::init_instruction(ooo_model_instr arch_instr) {
  ...
  if (instrs_to_read_this_cycle == 0)
    instrs_to_read_this_cycle = std::min(
        FETCH_WIDTH, (uint64_t)IFETCH_BUFFER.SIZE - IFETCH_BUFFER.occupancy);
  ...
  // ==================== 【新增代码：在解压读取 Trace 时直接统计间隔】
  // ====================
  // if (arch_instr.is_memory) {
  //   if (last_reader_mem_instr_id != 0) { // 排除全生命周期的第一条访存指令
  //     // 计算指令 ID 的差值，instr_unique_id 就是当前指令的绝对编号
  //     uint64_t interval = instr_unique_id - last_reader_mem_instr_id;

  //     // 累加总间隔和次数，用于计算平均值
  //     total_reader_mem_interval += interval;
  //     reader_mem_count++;

  //     // 直方图统计 (以 10 为步长)
  //     uint64_t bin = (interval / 10) * 10;
  //     reader_mem_interval_hist[bin]++;
  //   }
  //   // 更新“上一次访存指令”的 ID
  //   last_reader_mem_instr_id = instr_unique_id;
  // }
  // ====================================================================================
...
void O3_CPU::fetch_instruction() {
  ...
  for (uint32_t i = 0; i < IFETCH_BUFFER.SIZE; i++) {
    ...
    auto way = std::find_if(
        dib_set.begin(), dib_set.end(), [ifb_entry](dib_entry_t x) {
          return x.valid && ((x.addr >> LOG2_BLOCK_SIZE) ==
                             (ifb_entry.ip >> LOG2_BLOCK_SIZE));
        });
    if (way != dib_set.end()) {
    ...
    if ((ifb_entry.translated == COMPLETED) && (ifb_entry.fetched == 0)) {
      ...
      */
      ...
      if (rq_index != -2) {
        ...
        for (uint32_t j = 0; j < IFETCH_BUFFER.SIZE; j++) {
          if ((IFETCH_BUFFER.entry[j].ip >> LOG2_BLOCK_SIZE) ==
                  (ifb_entry.ip >> LOG2_BLOCK_SIZE) &&
              (IFETCH_BUFFER.entry[j].fetched == 0)) {
  ...
  while (checked_for_decode < DECODE_WIDTH && IFETCH_BUFFER.occupancy > 0 &&
         DECODE_BUFFER.occupancy < DECODE_BUFFER.SIZE &&
         IFETCH_BUFFER.entry[IFETCH_BUFFER.head].translated == COMPLETED &&
         IFETCH_BUFFER.entry[IFETCH_BUFFER.head].fetched == COMPLETED) {

    // ==================== 【新增代码：统计 Trace 访存间隔】
    // ====================
    // ooo_model_instr &current_instr = IFETCH_BUFFER.entry[IFETCH_BUFFER.head];

    // // 1. 判断是否为访存指令 (检查 source_memory 和 destination_memory 数组)
    // bool is_memory_instr = false;
    // for (int m = 0; m < NUM_INSTR_SOURCES; m++) {
    //   if (current_instr.source_memory[m] != 0) {
    //     is_memory_instr = true;
    //     break;
    //   }
    // }
    // if (!is_memory_instr) {
    //   for (int m = 0; m < NUM_INSTR_DESTINATIONS; m++) {
    //     if (current_instr.destination_memory[m] != 0) {
    //       is_memory_instr = true;
    //       break;
    //     }
    //   }
    ...
    // // 2. 如果是访存指令，计算间隔并更新统计信息
    // if (is_memory_instr) {
    //   if (last_trace_mem_instr_id != 0) { // 排除全生命周期的第一条访存指令
    //     // 计算指令 ID 的差值，即 Trace 中的指令间隔
    //     uint64_t interval = current_instr.instr_id - last_trace_mem_instr_id;

    //     // 累加总间隔和次数，用于计算平均值
    //     total_trace_mem_interval += interval;
    //     trace_mem_count++;

    //     // 直方图统计 (以 10 为步长，例如 0-9 归入 0，10-19 归入 10)
    //     uint64_t bin = (interval / 10) * 10;
    //     trace_mem_interval_hist[bin]++;
    //   }
    //   // 更新“上一次访存指令”的 ID
    //   last_trace_mem_instr_id = current_instr.instr_id;
    // }
    // ====================
    // ADD to decode buffer
    DECODE_BUFFER.entry[DECODE_BUFFER.tail] =
        IFETCH_BUFFER.entry[IFETCH_BUFFER.head];
    DECODE_BUFFER.entry[DECODE_BUFFER.tail].event_cycle =
...
void O3_CPU::decode_and_dispatch() {
  ...
  while (count_dispatches < DECODE_WIDTH && DECODE_BUFFER.occupancy > 0 &&
         ROB.occupancy < ROB.SIZE &&
         (!warmup_complete[cpu] ||
          ((DECODE_BUFFER.entry[DECODE_BUFFER.head].decoded) &&
...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr) {
  ...
  if (L1I.PQ.occupancy < L1I.PQ.SIZE) {
    ...
    uint64_t pf_pa =
        (vmem.va_to_pa(cpu, pf_v_addr) & (~((1 << LOG2_PAGE_SIZE) - 1))) |
...
void O3_CPU::do_scheduling(uint32_t rob_index) {
  ...
  if (ROB.entry[rob_index].is_memory)
    ...
    if (ROB.entry[rob_index].reg_ready) {
      ...
      DP(if (warmup_complete[cpu]) {
        cout << "[ready_to_execute] " << __func__
             << " instr_id: " << ROB.entry[rob_index].instr_id
             << " rob_index: " << rob_index << " is added to ready_to_execute";
        cout << " head: " << ready_to_execute_head
             << " tail: " << ready_to_execute_tail << endl;
      });
...
void O3_CPU::execute_instruction() {
  ...
  while (exec_issued < EXEC_WIDTH) {
    if (ready_to_execute[ready_to_execute_head] < ROB_SIZE) {
      ...
      // DP (if (warmup_complete[cpu]) {
      // cout << "[ready_to_execute] is empty head: " << ready_to_execute_head
      // << " tail: " << ready_to_execute_tail << endl; });
      break;
    }
  ...
  if (ROB.head < ROB.tail) {
    for (uint32_t i = ROB.head; i < ROB.tail; i++) {
      ...
      if (ROB.entry[i].is_memory && mem_reg_dependence_resolved(i) &&
        ...
        // ==================== 【修改 1：新增统计乱序调度周期间隔】
        // ====================
        // 第一层判断：确保这条指令确确实实被成功调度进队列了
        // if (ROB.entry[i].scheduled == COMPLETED) {
        //   // 第二层判断：排除全生命周期的第一条指令
        //   if (last_exec_mem_cycle != 0) {
        //     uint64_t interval = current_core_cycle[cpu] -
        //     last_exec_mem_cycle; total_exec_mem_interval += interval;
        //     exec_mem_count++;
        //     uint64_t bin = (interval / 10) * 10;
        //     exec_mem_interval_hist[bin]++;
        //   }
        //   last_exec_mem_cycle = current_core_cycle[cpu];
        // }
        // ==============================================================================
      }
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
  ...
  if (is_it_tlb) {
    ...
    auto way =
        std::find_if_not(dib_set.begin(), dib_set.end(), [](dib_entry_t x) {
          return x.valid;
        }); // search for invalid
    if (way == dib_set.end())
      way = std::max_element(dib_set.begin(), dib_set.end(),
                             [](dib_entry_t x, dib_entry_t y) {
                               return x.lru < y.lru;
                             }); // search for LRU
