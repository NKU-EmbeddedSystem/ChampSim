...
void print_roi_stats(uint32_t cpu, CACHE *cache) {
  ...
  cout << cache->NAME << " MSHR FULL STALL CYCLES: " << cache->mshr_full_stalls
       << endl;
...
void finish_warmup() {
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    ...
    // ==================== 【新增：清空预热阶段的访存间隔统计】
    // ====================
    // ooo_cpu[i].last_reader_mem_instr_id = 0;
    // ooo_cpu[i].total_reader_mem_interval = 0;
    // ooo_cpu[i].reader_mem_count = 0;
    // ooo_cpu[i].reader_mem_interval_hist.clear();

    // ooo_cpu[i].last_trace_mem_instr_id = 0;
    // ooo_cpu[i].total_trace_mem_interval = 0;
    // ooo_cpu[i].trace_mem_count = 0;
    // ooo_cpu[i].trace_mem_interval_hist.clear();

    // ooo_cpu[i].last_exec_mem_cycle = 0;
    // ooo_cpu[i].total_exec_mem_interval = 0;
    // ooo_cpu[i].exec_mem_count = 0;
    // ooo_cpu[i].exec_mem_interval_hist.clear();
    // ==============================================================================
...
int main(int argc, char **argv) {
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    ...
    // ==================== 【在这里插入你的统计打印代码】 ====================
    // double avg_reader_interval =
    //     ooo_cpu[i].reader_mem_count > 0
    //         ? (double)ooo_cpu[i].total_reader_mem_interval /
    //               ooo_cpu[i].reader_mem_count
    //         : 0;
    // double avg_trace_interval =
    //     ooo_cpu[i].trace_mem_count > 0
    //         ? (double)ooo_cpu[i].total_trace_mem_interval /
    //               ooo_cpu[i].trace_mem_count
    //         : 0;
    // double avg_exec_interval =
    //     ooo_cpu[i].exec_mem_count > 0
    //         ? (double)ooo_cpu[i].total_exec_mem_interval /
    //               ooo_cpu[i].exec_mem_count
    //         : 0;

    // cout << "=== CPU " << i << " Memory Instruction Interval Stats ===" <<
    // endl; cout << "Average Reader Interval (Raw Trace):     " <<
    // avg_reader_interval
    //      << " Instructions" << endl;
    // cout << "Average Fetch Interval (After Pipeline): " << avg_trace_interval
    //      << " Instructions" << endl;
    // cout << "Average Execution Interval (Cycles):     " << avg_exec_interval
    //      << " Cycles" << endl;

    // // 新增：打印三个阶段的总间隔（分子）
    // cout << "Total Reader Interval (Instructions):    "
    //      << ooo_cpu[i].total_reader_mem_interval << endl;
    // cout << "Total Fetch Interval (Instructions):     "
    //      << ooo_cpu[i].total_trace_mem_interval << endl;
    // cout << "Total Execution Interval (Cycles):       "
    //      << ooo_cpu[i].total_exec_mem_interval << endl;

    // // 打印总数，方便你直接对比解压时和进入流水线后的访存指令数量差异（分母）
    // cout << "Total Mem Instructions (Reader): " <<
    // ooo_cpu[i].reader_mem_count
    //      << endl;
    // cout << "Total Mem Instructions (Fetch):  " << ooo_cpu[i].trace_mem_count
    //      << endl;
    // cout << "Total Mem Instructions (Exec):   " << ooo_cpu[i].exec_mem_count
    //      << endl;

    // // 3. 分别打印三个直方图
    // cout << "\nReader Interval Histogram (Raw Trace, Bin Size = 10):" <<
    // endl; for (const auto &pair : ooo_cpu[i].reader_mem_interval_hist) {
    //   cout << "Bin [" << pair.first << " - " << pair.first + 9
    //        << "] : " << pair.second << " times" << endl;
    // }

    // cout << "\nTrace (Fetch) Interval Histogram (Bin Size = 10):" << endl;
    // for (const auto &pair : ooo_cpu[i].trace_mem_interval_hist) {
    //   cout << "Bin [" << pair.first << " - " << pair.first + 9
    //        << "] : " << pair.second << " times" << endl;
    // }

    // cout << "\nExecution Interval Histogram (Bin Size = 10):" << endl;
    // for (const auto &pair : ooo_cpu[i].exec_mem_interval_hist) {
    //   cout << "Bin [" << pair.first << " - " << pair.first + 9
    //        << "] : " << pair.second << " times" << endl;
    // }
    // =========================================================================
